...
function printDependencies(fileDependencies) {
  fileDependencies.forEach(({ filePath, contractNames, imports }) => {
    ...
    const dependentFiles = fileDependencies.filter((fileDep) =>
      fileDep.imports.some((importPath) =>
        importPath.endsWith(normalizedFilePath)
      )
    );
    ...
    if (dependentFiles.length > 0) {
      ...
      dependentFiles.forEach(
        ({
          filePath: dependentFilePath,
          contractNames: dependentContractNames,
        }) => {
...
function saveToXLSX(fileDependencies) {
  ...
  const worksheetData = [
    ["Contract Name", "Dependencies", "Dependent Contracts"],
  ];
  ...
  fileDependencies.forEach(({ filePath, contractNames, imports }) => {
    ...
    contractNames.forEach((contractName) => {
      worksheetData.push([contractName, imports.length, dependentFiles.length]);
    });
  });
...
module.exports = { exportXlsxCouplingsData };
